  ...
  private RobotContainer() {
    ...
    m_driveTrain.setDefaultCommand(
      new Teleop(
        ...
        () -> -0.28 * driveController.getRawAxis(4),
        () -> m_upAndDown.getLiftPosition(),
        m_driveTrain));
   ...
   // m_wrist.setDefaultCommand(new TeleopWrist(
    ...
    // m_chooser.addOption("Drive Forward Auto", new DriveAuto(m_driveTrain));
    // m_chooser.addOption("Charge Station Auto", new ChargeStationAuto(m_driveTrain));
    // m_chooser.addOption("Eject Auto", new EjectAuto(m_intake).andThen(new DriveAuto(m_driveTrain)));
   
    //TODO: Ad buttons for each level to score
    //auto: 1. score highest level 2. balance
    m_chooser.addOption("Lift High Mid Pos Auto",
      new SequentialCommandGroup(new AutoLift(m_upAndDown, Constants.kHighLift), 
      new EjectAuto(m_intake),
      //new ParallelCommandGroup(
        new AutoLift(m_upAndDown, Constants.kLowLift),
        new DriveAuto(m_driveTrain, -2.2)
      //)
      ,
    ...
    m_chooser.addOption("Balance",
      new SequentialCommandGroup(new DriveAuto(m_driveTrain, -2.0), new Balance(m_driveTrain))
    );

    m_chooser.addOption("Balance + Eject Cube",
      new SequentialCommandGroup(new EjectAuto(m_intake), new DriveAuto(m_driveTrain, -2.2), new Balance(m_driveTrain))
    );
    
    m_chooser.addOption("Drive Backwards Only",
      new DriveAuto(m_driveTrain, -4.0));

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=AUTONOMOUS

    m_chooser.addOption("Eject Cube",
      new SequentialCommandGroup(new EjectAuto(m_intake), new DriveAuto(m_driveTrain, -4.0))
